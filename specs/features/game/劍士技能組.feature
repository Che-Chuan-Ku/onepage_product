...
Feature: 劍士技能組
  ...
  # 規則（Q4、Q10 已定；縱劈依 2026-07-07 規則變更改為一格寬）：
  #   橫劈：附掛落子，以該子為基準選上或下，緊鄰該方向橫排 3 格（正前+左前+右前）往該方向推 1 格；
  #   縱劈：附掛落子，以該子為基準選左或右，只推「緊鄰該方向的那一格」往該方向 1 格（一格寬）；
  ...
  Rule: 前置（參數）- 縱劈以落子為基準選擇左或右方向，只推緊鄰該方向的一格（一格寬）

    Example: 縱劈選擇 "LEFT" 方向推擠緊鄰左側一格
      When 玩家 "alice" 於 (7,7) 落子並附掛 "VERTICAL_SLASH"，方向 "LEFT"
      Then 系統對 (7,6) 這一格施加往左推 1 格
      And 系統發布 SkillUsed 與 StonesPushed 事件
  ...
  # Q4/Q10 已定：3寬x2深＝含目標格橫排3+其後一排共6格；錨點必須空格；橫劈方向與範圍如上。
  # 縱劈規則變更（2026-07-07 使用者試玩後裁決，documents/clarify/2026-07-04-1021.md 末段）：
  #   縱劈改為一格寬，只推落子格緊鄰所選方向（左/右）那一格；連鎖推擠/出界移除/障礙擋停照舊。
