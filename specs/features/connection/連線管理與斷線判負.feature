...
Feature: 連線管理與斷線判負
  ...
  Rule: 後置（狀態）- 真劍勝負對局重連後，技能使用狀態與場地狀態隨對局狀態一併恢復（需求 #46）

    Example: 真劍勝負對局斷線重連後恢復技能與場地狀態
      Given 玩家 "alice" 於真劍勝負對局（場地 "VOLCANO"）中斷線
      And 斷線前 "alice" 已使用技能 "HORIZONTAL_SLASH"，場地已生成障礙物與已觸發噴發格
      When 玩家 "alice" 在 30 秒內重新連線
      Then 系統發布 PlayerReconnected 事件
      And "alice" 的技能已用狀態（HORIZONTAL_SLASH 已用）隨對局狀態恢復
      And 場地狀態（障礙物、已觸發噴發格、回合計數）隨對局狀態恢復

  # 需求 #46 已定（R2-3）：技能已用狀態與場地狀態沿用既有重連機制一併恢復。
